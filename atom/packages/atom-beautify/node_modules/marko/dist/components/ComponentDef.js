...
function ComponentDef(component, componentId, globalComponentsContext, componentStack, componentStackLen) {
    this._i_ = globalComponentsContext; // The AsyncWriter that this component is associated with
    this._j_ = componentStack;
    this._k_ = componentStackLen;
    this.h_ = component;
    this.id = componentId;

    this.s_ = null; // IDs of root elements if there are multiple root elements
    this._l_ = null; // An array of nested ComponentDef instances
    this._m_ = undefined; // An array of DOM events that need to be added (in sets of three)

    this._n_ = false;

    this._o_ = false;

    this._p_ = 0; // The unique integer to use for the next scoped ID
}

ComponentDef.prototype = {
    _q_: function () {
        this._j_.length = this._k_;
    },
     ...
     */
    _r_: function (componentDef) {
        var children = this._l_;

        if (children) {
            children.push(componentDef);
        } else {
            this._l_ = [componentDef];
        }
    },
    /**
    ...
    elId: function (nestedId) {
        ...
        if (nestedId == null) {
            ...
            if (typeof nestedId == 'string' && repeatedRegExp.test(nestedId)) {
                return this._i_._s_(id, nestedId);
            } else {
                return id + '-' + nestedId;
            }
        }
    ...
    e: function (type, targetMethod, elId, extraArgs) {
        if (targetMethod) {
            ...
            (this._m_ || (this._m_ = [])).push([type, targetMethod, elId, extraArgs]);
        }
    },
    /**
    ...
    _t_: function () {
        ...
        return id === null ? this._i_._t_(this.q_) : id + '-c' + this._p_++;
    },
...
ComponentDef._u_ = function (o, types, globals, registry) {
    var id = o[0];
    ...
    var component = typeName /* legacy */ && registry._v_(typeName, id);
