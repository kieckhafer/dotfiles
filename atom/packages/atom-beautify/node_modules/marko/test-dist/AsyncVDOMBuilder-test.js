...
function getChildNodes(parentNode) {
    ...
    var curChild = parentNode._a_;
    while (curChild) {
        childNodes.push(curChild);
        curChild = curChild._b_;
    }
...
describe('AsyncVDOMBuilder', function () {
    ...
    it('async', function (done) {
        ...
        out.on('finish', function (result) {
            ...
            expect(tree._a_.aa_).to.equal('div');
            expect(tree._a_._b_.aa_).to.equal('span');
            expect(tree._a_._b_._b_.aa_).to.equal('section');
            done();
        });
    });
    ...
    it('for loop', function (done) {
        ...
        out.once('finish', function (result) {
            ...
            expect(header.aa_).to.equal('h1');
            expect(list.aa_).to.equal('ul');
            expect(getChildNodes(list).length).to.equal(10);
            expect(paragraph.aa_).to.equal('p');
            done();
        });
    ...
    it('staticNode, text, comment', function (done) {
        ...
        out.once('finish', function (result) {
            var tree = result.getOutput();
            var childNodes = getChildNodes(tree);
            expect(childNodes[0].aa_).to.equal('div');
            expect(childNodes[1].ae_).to.equal('Hello <em>World</em>');
            expect(childNodes[2].ae_).to.equal('TODO: make this work');
            done();
        });
    });
